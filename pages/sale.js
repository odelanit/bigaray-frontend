...
class Homepage extends Component {
    ...
    async fetchData() {
        ...
        let data = []
        data = await axios.get(`${config.domain}/backend/api/homepage-data?page=0&site_type=2`, { withCredentials: true, headers })
        if (!data.data) {
    ...
    loadMoreData = async () => {
        ...
        let data = await axios.get(`${config.domain}/backend/api/homepage-data?page=${this.state.dataPage}&site_type=2`, { withCredentials: true, headers })
        data = shuffle(data.data.data)
