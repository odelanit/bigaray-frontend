...
class Homepage extends Component {
    ...
    async fetchData() {
        ...
        console.log('fetchData', this.state.exploreAll);

        let data = []
        data = await axios.get(`${config.domain}/backend/api/homepage-data?page=0&site_type=1&all=${this.state.exploreAll}`, {
    ...
    loadMoreData = async () => {
        ...
        let data = await axios.get(`${config.domain}/backend/api/homepage-data?page=${this.state.dataPage}&site_type=1&all=${this.state.exploreAll}`, {
            withCredentials: true,
            headers
        })
        data = shuffle(data.data.data)
