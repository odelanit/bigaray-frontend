...
class Homepage extends Component {
    ...
    async fetchData() {
        ...
        let data = []
        data = await axios.get(`${config.domain}/backend/api/homepage-data?page=0&site_type=${this.state.filterBy}`, { withCredentials: true, headers })
        if (!data.data) {
    ...
    loadMoreData = async () => {
        ...
        let data = await axios.get(`${config.domain}/backend/api/homepage-data?page=${this.state.dataPage}&site_type=${this.state.filterBy}`, { withCredentials: true, headers })
        data = shuffle(data.data.data)
                                    ...
                                    <Sticky enabled={this.state.stickyNav} top={0} bottomBoundary={0} innerZ={1500} activeClass={'sticky-active'} releasedClass={'sticky-released'} >
                                        <div className="sticky-container" style={{ backgroundColor: 'white', width: '98%' }}>
                                            <span
                                                ...
                                                className={`${this.state.filterBy === 1 && "has-text-weight-bold"}`}>
                                                New Arrivals</span>
                                            ...
                                            <span
                                                ...
                                                style={{ cursor: 'pointer', marginRight: '10px', fontSize: '20px', color: 'black' }}
                                                className={`${this.state.filterBy === 2 && "has-text-weight-bold"}`}
                                            >Sale</span>
